<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="sidenav">
    <nav class="box">
      <ul>
        ...
        <xsl:if test="$method/pdx:parameter">
          <li><a href="#parameter">Parámetros</a></li>
        </xsl:if>

        <xsl:if test="$method/pdx:docblock//pdx:return">
          <li><a href="#return">Devolución</a></li>
        </xsl:if>
        ...
        <xsl:if test="$unit/@start"><!-- hack: test for start line == we know something about this class -->
          <li><a href="{$base}source/{$unit/pdx:file/@relative}.{$extension}#line{$method/@start}">Código Fuente</a></li>
        </xsl:if>
      </ul>
    </nav>
  </xsl:template>
  ...
  <xsl:template name="parameterlist">
    <h2 id="parameterlist">Parámetros</h2>
    <dl class="styled">
  ...
  <xsl:template name="throws">
    <h2 id="throws">Errores/Excepciones</h2>
    <dl class="styled">
      <xsl:for-each select="$method/pdx:docblock//pdx:throws">
        <dt>
          <code><xsl:copy-of select="pdxf:link(pdx:type, '', pdx:type/@name)" /></code>
        </dt>
        <dd>
  ...
  <xsl:template name="interface">
    <h2 id="interface">Definida por la interfaz</h2>
    <p class="styled">
  ...
  <xsl:template name="tests">
    <h2 id="tests">Cobertura de errores</h2>
    <div class="styled">
      ...
      <h3>Información</h3>
      <ul class="styled">
        <li>Cobertura: <xsl:value-of select="$coverage/@executed"/>/<xsl:value-of select="$coverage/@executable"/> Líneas (<xsl:value-of select="$coverage/@coverage"/>%)</li>
        <li>Tests: <xsl:value-of select="$count" /></li>
        <li>Aprobados: <xsl:value-of select="$passed" /> (<xsl:choose>
