<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="title" select="'Clases'" />
            ...
            <tr>
              ...
              <td>
                <xsl:choose>
                  ...
                  <xsl:otherwise><span class="unavailable">Descripción no disponible</span></xsl:otherwise>
                </xsl:choose>
              </td>
              <xsl:if test="$mode = 'class'">
                <td>
      ...
      <xsl:when test="$class//pu:coverage/@coverage != 0 or $class//pu:coverage/@executable != 0">
        ...
        <xsl:choose>
          ...
          <xsl:when test="sum($result/@*) = 0 or $result/@skipped != 0 or $result/@incomplete != 0">
            <xsl:attribute name="class">testresult-SKIPPED</xsl:attribute>UNTESTED
          </xsl:when>

          <!-- at least one is failure or error-->
          <xsl:when test="$result/@failure != '0' or $result/@error != '0'">
            <xsl:attribute name="class">testresult-FAILED</xsl:attribute>FAILED
          </xsl:when>

          <!-- everything 0 except passed -->
          <xsl:when test="sum($result/@*) = $result/@passed and $result/@passed != 0">
            <xsl:attribute name="class">testresult-PASSED</xsl:attribute>PASSED
          </xsl:when>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
